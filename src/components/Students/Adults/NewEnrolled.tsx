...
import Done from "@/assets/images/shared/dossierThree.svg"
...
{courses.map((course,index)=>{
        return(
             <div>
                {
                    course.status==="completed" ? <div className='flex items-center flex-col'><div onClick={()=>handleDossierClick(`chapitre${index}`)}><Image src={Done}  alt="dossier" width={140} height={135}/></div><h1 className='text-[#6DDB71] text-xl font-bold mt-5'>Chapitre {index}</h1></div> : <div className='flex items-center flex-col'><div onClick={()=>handleDossierClick(`chapitre${index}`)}><Image src={Dossier}  alt="dossier" width={140} height={135}/></div><h1 className='text-[#896DDB] text-xl font-bold mt-5'>Chapitre {index}</h1></div> 
